...
namespace LocresLib
{
    public class LocresFile : Dictionary<string, LocresNamespace>
    {
        ...
        {
            Save(stream, LocresVersion.Compact);
        }

        public void Save(Stream stream, LocresVersion outputVersion)
        {
            if (!stream.CanSeek)
            {
                throw new ArgumentException("Stream must be seekable.");
            ...
            using (BinaryWriter w = new BinaryWriter(stream))
            {
                ...
                foreach (var localizationNamespace in Values)
                {
                    ...
                    {
                        w.Write(Crc.StrCrc32(localizationNamespace.Name, 0));
                    }
                    ...
                    foreach (var localizedString in localizationNamespace.Values)
                    {
                        ...
                        {
                            w.Write(Crc.StrCrc32(localizedString.Key, 0));
                        }
